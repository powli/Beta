            ...
            _manager.CreateCommands("", cgb =>
            {
                ...
                cgb.CreateCommand("reg")
                    ...
                    .Do(async e =>
                    {
                        ...
                        {
                            case "xbl":
                                Beta.GamertagRepository.NewGamertag(e.Args[1].Trim(), "Xbox Live", e.Message.User.Name, e.Message.User.Id);
                                await e.Channel.SendMessage("Added that gamertag for you, " + e.Message.User.Name + ".");
                                break;
                            case "psn":
                                Beta.GamertagRepository.NewGamertag(e.Args[1].Trim(), "Playstation Network", e.Message.User.Name, e.Message.User.Id);
                                await e.Channel.SendMessage("Added that gamertag for you, " + e.Message.User.Name + ".");
                                break;
                            case "nnid":
                                Beta.GamertagRepository.NewGamertag(e.Args[1].Trim(), "Nintendo Network", e.Message.User.Name, e.Message.User.Id);
                                await e.Channel.SendMessage("Added that gamertag for you, " + e.Message.User.Name + ".");
                                break;
                            case "steam":
                                Beta.GamertagRepository.NewGamertag(e.Args[1].Trim(), "Steam", e.Message.User.Name, e.Message.User.Id);
                                await e.Channel.SendMessage("Added that gamertag for you, " + e.Message.User.Name + ".");
                                break;
                            case "b.n":
                                Beta.GamertagRepository.NewGamertag(e.Args[1].Trim(), "Battle.Net", e.Message.User.Name, e.Message.User.Id);
                                await e.Channel.SendMessage("Added that gamertag for you, " + e.Message.User.Name + ".");
                                break;
                            default:
                                await e.Channel.SendMessage("Sorry, I don't recognize that type of gamertag. I currently recognize the following types of Gamertags (With explination in parenthesis): ");
                                await e.Channel.SendMessage("XBL (Xbox Live), PSN (Playstation Network), NNID (Nintendo Network), Steam, B.N (Battle.Net)");
                                break;
                        }
                    });
                ...
                cgb.CreateCommand("list")                    
                    ...
                    .Do(async e =>
                    {
                        ...
                        if (Beta.GamertagRepository.Gamertags.FirstOrDefault(g => g.GamertagType == "Battle.Net") != null)
                        {
                        ...
                        if (Beta.GamertagRepository.Gamertags.FirstOrDefault(g => g.GamertagType == "Xbox Live") != null)
                        {
                        ...
                        if (Beta.GamertagRepository.Gamertags.FirstOrDefault(g => g.GamertagType == "Playstation Network") != null)
                        {
                        ...
                        if (Beta.GamertagRepository.Gamertags.FirstOrDefault(g => g.GamertagType == "Nintendo Network") != null)
                        {
                ...
                cgb.CreateCommand("retrieve")
                    ...
                    .Do(async e =>{
                        ...
                        foreach (Gamertag tag in Beta.GamertagRepository.Gamertags)
                        {
                            ...
                            {
                                found = true;
                                retrieved +="Discord Username: **"+tag.DiscordUsername+"** Gamertag: **"+tag.GTag+"** Network: **"+tag.GamertagType+"** Gamertag ID: **"+tag.ID+"**\n";
                            }                            
                            
                        }
                        if (found) await e.Channel.SendMessage(retrieved);
                        else await e.Channel.SendMessage("Sorry, I didn't see any gamertags for that person.");
                    });
                        ...
                        if ( Int32.TryParse(e.GetArg("ID"), out id) )
                        {
                            ...
                            if (removeTag != null)
                            {
                                if (removeTag.DiscordID == e.Message.User.Id)
                                {
                                    ...
                                    await e.Channel.SendMessage("Ok cool I've removed that gamertag for you! ");
                                    Beta.GamertagRepository.Save();
                                }
                                else await e.Channel.SendMessage("Hey that's not yours! Leave it alone, dude.");
                            }
                            else
                            {
                                await e.Channel.SendMessage("Sorry, I didn't find that gamertag. Are you sure you have the right number?");
                            }
                        }
